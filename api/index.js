...
const feishuClient = new BaseClient({ appToken: FEISHU_APP_TOKEN, personalBaseToken: FEISHU_PERSONAL_BASE_TOKEN });
...
app.post('/api/analyze', async (req, res) => {
    ...
    if (!record_id) return res.status(400).json({ error: '请求体中必须包含 record_id' });

    try {
        await feishuClient.base.appTableRecord.update({ path: { table_id: TABLE_ID_TASKS, record_id: record_id }, data: { fields: { '分析状态': '分析中' } } });
        const taskRecordResponse = await feishuClient.base.appTableRecord.get({ path: { table_id: TABLE_ID_TASKS, record_id: record_id } });
        ...
        for (const personaId of personaRecordIds) {
            const personaRecord = await feishuClient.base.appTableRecord.get({ path: { table_id: TABLE_ID_PERSONAS, record_id: personaId } });
            const personaDesc = personaRecord.data.record.fields['AI提示词描述'];
            if (!personaDesc) continue;

            const model = genAI.getGenerativeModel({ model: GEMINI_MODEL_IDENTIFIER });
            const prompt = `你是一名顶级的短视频内容策略师。请严格且仅仅从以下 [人物画像] 的视角，对提供的视频进行深入分析。
            ...
            await feishuClient.base.appTableRecord.create({ data: { table_id: TABLE_ID_RESULTS, fields: { '关联任务': [{ record_id: record_id }], '分析视角': [{ record_id: personaId }], '时间点分析': analysisResult.timestamp_analysis, '综合评价与建议': analysisResult.overall_summary } } });
        }

        await feishuClient.base.appTableRecord.update({ path: { table_id: TABLE_ID_TASKS, record_id: record_id }, data: { fields: { '分析状态': '已完成' } } });
